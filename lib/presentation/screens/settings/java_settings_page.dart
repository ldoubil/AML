...
import 'package:aml/services/java_download_service.dart';
...
class _JavaSettingsPageState extends State<JavaSettingsPage> {
  ...
  Future<void> _searchJava(int version) async {
    try {
      final javaVersion = await JavaDownloadService.checkJavaInstallation();
      if (javaVersion != null) {
  ...
  Future<void> _testJava(int version) async {
    ...
    try {
      ...
      final isValid = await JavaDownloadService.testJRE(javaPath, version);
      if (isValid) {
